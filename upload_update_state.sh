...
#   0. Make sure we have no left over PUDB statements
...
FOLDER=upload_update_state

#   0. Make sure we have no left over PUDB statements
pudb_lines=$(grep -R --exclude=upload_trade.sh --upload_update_state.sh --exclude=to_upload.zip PUDB)
...
echo 4
#   4. Add pip packages
for package in {bittrex,pandas,python-gflags}; do
