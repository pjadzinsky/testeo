...
#
# The script is name <upload_<script>.sh and the aws lambda function will be name just <script> and the code
# it will run will be in s3://my-lambda-func/<script>.zip
#
...
cp ../choose_currencies.py $FOLDER
cp ../config.py $FOLDER
...
echo 6
#   6. upload to s3
aws s3 cp ${LAMBDA}.zip s3://my-lambda-func/${LAMBDA}.zip --profile pablo
